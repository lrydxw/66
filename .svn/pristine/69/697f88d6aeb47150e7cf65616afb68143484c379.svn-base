...
$(function () {
    ...
    var tradeFun = transfar.Base.extend({
        ...
        switchStatus: function () {
            ...
            // var source2 = '<td>{{d.bidtypeWord}}</td>' + '<td>{{d.from}}--{{d.to}}</td>' + '<td>{{d.goodsname}}</td>' + '<td>{{d.goodsSpaceW}}<br/>{{d.goodsSpaceV}}</td>' + '<td>{{d.inputdate}}</td>' + '<td>{{d.ownerlinkman}}</td>' + '<td>{{d.ownerlinkphone}}</td>' + '<td>{{d.total }}</td>';
            ...
            switch (this.type) {
                ...
                case "send":
                    ...
                    $("#subTilte").html("已转运订单")
                     $("#agentControll").hide();
                    $("#orderStatus").val(2);
                    // console.log(d.dispatchorderfee[0]["updatedate"])
                    $("#packageStatus").show();
                    $('#finishTime').hide();
                    $('#driverInfo').hide();
                    // this.source += "<td>{{d.statusName}}</td>" + source2
                    this.source += 
                    '<td>{{d.outcode}}</td>' +
