...
$(function () {
    ...
    var tradeFun = transfar.Base.extend({
        ...
        switchStatus: function () {
            ...
            '<td>应收:{{d.feein}}<br/>应付:{{d.feeout}}&nbsp;&nbsp;</td>' + 
            '<td>{{d.inputdate}}</td>'  + 
            '<td style="display:{{d.noSend}}">'+
            '<a href="./carOrder.html?type=carOrderAgent&id={{d.fromordercode||d.ordercode}}" >查看派车单</a>'+
            '</td>';
                    break;

                case "success":
                    $("title").html("陆鲸-承运商-完成订单")
                    $("#subTilte").html("完成订单")
                    $("#orderStatus").val(2)
                    //$("#agentControll").hide();
                    // $("#finishTime").hide();
